	...
	<fieldset class="row setup-content">
		<div class="modal-body" id="addQuestionFormControls">
			<div class="form-group">
				<label for="questions_01" class="col-form-label">Question {{$qnno}}:</label>
				<textarea class="form-control" rows="4"  name="question" id="question"></textarea>
				<input type="hidden" name="qnno" value="{{$qnno}}" />
			</div>
		...
		<div class="modal-footer d-flex justify-content-between">
			<button id="saveQn" route="{{route('quiz.store.questions', $id)}}" disabled="true" type="button" class="btn btn-primary btn-action">Save and Quit</button>
			<button id="saveAndContQn" route="{{route('quiz.storeAndContinue.questions', $id)}}" disabled="true" class="btn btn-action nextBtn btn-success" type="button">Save and Continue</button>
		</div>
	</fieldset>
</form>
